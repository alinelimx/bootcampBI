# Business Intelligence — WoMakersCode + Instituto Localiza

Repositório criado para reunir meus exercícios, atividades e projetos desenvolvidos durante o Bootcamp Business Intelligence (Turma 2025.2) da WoMakersCode em parceria com o Instituto Localiza ( https://womakerscode.org/bi-instituto-localiza )

Um programa de formação intensiva e gratuita voltado para mulheres na área de dados e tecnologia, com duração de 6.5 meses, mentorias de carreira e conexões reais com o mercado 🚀

Aprendendo a transformar dados em decisões inteligentes — da coleta e modelagem à visualização e storytelling 📊

---
### 🎯 Objetivo
Desenvolver competências técnicas e analíticas para atuar na área de Business Intelligence, aplicando conceitos de dados, visualização e inteligência artificial em soluções práticas que apoiem a tomada de decisões estratégicas.

---
### 🧠 O que estou aprendendo
- Fundamentos de Business Intelligence
- SQL e Bancos de Dados relacionais e não relacionais
- Processos de ETL (Extração, Transformação e Carga)
- Python para análise e tratamento de dados
- Criação de dashboards e relatórios interativos com Power BI e Tableau
- Inteligência Artificial aplicada ao BI
- Governança, qualidade e ética dos dados

---
### 🧩 Habilidades Técnicas
- Governança e Qualidade de Dados
- ETL e Modelagem de Dados
- Power BI / Tableau
- Computação em Nuvem
- IA aplicada ao BI

[![Skills](https://skillicons.dev/icons?i=python,mysql,github,vscode,powerbi)](https://skillicons.dev)
---
### 💬 Soft Skills e Carreira
🗣 Comunicação e Apresentação de Dados

🧩 Pensamento Analítico e Resolução de Problemas

🌱 Aprendizado Contínuo e Autonomia

🤝 Liderança e Trabalho em Equipe

💡 Planejamento e Desenvolvimento de Carreira

👩‍💻 Inclusão, Diversidade e Protagonismo Feminino na Tecnologia


 
